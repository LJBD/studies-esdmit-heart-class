function [groups, centers, ad] = gmeans(data, ad_threshold, iterations)
...
if ( nargin < 2 )
    error('Syntax : [groups, centers, ad] = cmeans(data, ad_threshold[, iterations])');
    return;
end
...
[pc, latent, explained] = pcacov(cov(data));
projected = pc(:,1)'*data';
...
while (ready == 0)
    ...
    % perform k means
    if ( iterations == 0 )
        [kout, tcenters] = kmeans( tdata, kmeansK, 'Start', ncenters, 'EmptyAction', 'singleton', ...
            'Display', 'off' );
    else
        [kout, tcenters] = kmeans( tdata, kmeansK, 'Replicates', iterations, 'EmptyAction', 'singleton', ...
            'Display', 'off' );
    end
    ncenters = [];
